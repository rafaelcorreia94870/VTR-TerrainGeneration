...
void main(){
	...
	*/
	...
	vec3 front_Height = vec3 (0+OFFSET,fbm(vec2(p.x+OFFSET,p.z)),0);
  	vec3 right_Height = vec3 (0,fbm(vec2(p.x,p.z+OFFSET)),0+OFFSET);
	vec3 back_Height = vec3 (0-OFFSET,fbm(vec2(p.x-OFFSET,p.z)),0);
	vec3 left_Height = vec3 (0,fbm(vec2(p.x,p.z-OFFSET)),0-OFFSET);
